...
const Home = () => {
  ...
  useEffect(() => {
    ...
    getAllProducts(0, 6).then((data) => {
      setFlashSale(data);
    }).catch((err) => {
      console.log(err);
    });

    getAllProducts(0, 4).then((data) => {
      setBestSelling(data);
    }).catch((err) => {
      console.log(err);
    });
  ...
  return (
    <Layout>
      <div className="flex md:flex-row flex-col container md:px-0 px-5">
        <div className="md:border-r border-neutral-300 py-6 gap-4 overflow-x-scroll flex md:flex-col md:w-[20%]">
          {categories && categories.map((category) => {
            return <button key={category.id} className="w-full text-start hover:font-bold">{category.name}</button>
          })}
        </div>
        <div className="md:p-6 md:w-[80%]">
          <div className="bg-black h-[200px] md:h-[300px]">
            sfa
          </div>
        </div>
      </div>
      ...
      <Section title="Categories">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
          {categories && categories.map((category) => {
            return <button key={category.id} className="border border-neutral-400 rounded-md hover:bg-red-500 hover:text-white h-20">{category.name}</button>
          })}
        </div>
      </Section>
      <Section title="This Month">
        <div className="flex items-center justify-between gap-10">
          <h1 className="font-bold text-xl md:text-3xl">Best Selling Products</h1>
      ...
      <Section title="Our Products">
        <div className="flex items-center justify-between gap-10">
          <h1 className="font-bold text-xl md:text-3xl">Explore Our Products</h1>
          {/* <button className="text-white bg-red-500 w-32 p-2 rounded-md self-center">
            View All
          </button> */}
        </div>
